...
const solicitarRestablecimiento = async (req, res) => {
  ...
  try {
    ...
    const credencialesGmail = {
      usuario: 'ecoplan38@gmail.com',  // Reemplaza con tu dirección de correo Gmail
      contrasena: 'qxiq fcaa lgfa vmnq',  // O utiliza una contraseña de aplicación
    };

    // Enviar correo electrónico con el enlace de recuperación que contiene el token
    await enviarCorreoRecuperacion(usuario.email, resetToken, credencialesGmail);
...
const enviarCorreoRecuperacion = async (correoDestino, token, credenciales) => {
  try {
    ...
    const usuario = await Usuario.findOne({ where: { email: correoDestino } });
